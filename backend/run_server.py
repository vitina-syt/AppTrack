...
This file is referenced by StepCast_backend.spec as the analysis target.
When run as a standalone exe it starts the FastAPI backend on the given port.

Usage:
    StepCast_backend.exe [--port 8001] [--frontend-dist path/to/dist]
"""
import sys
import os
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="StepCast backend server")
    ...
    if args.frontend_dist:
        os.environ["StepCast_FRONTEND_DIST"] = args.frontend_dist
